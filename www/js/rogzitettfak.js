myApp.onPageBeforeInit('rogzitettfak', function (page) {
    $("#falista").html("");
    currentPage = "rogzitettfak";
    var trees, items = [];
    $.getJSON(apiaddress + "?action=FelvittLista&auth_token=" + authenticate.auth_token + "&neptun=" + authenticate.neptun + "&szektor=" + authenticate.szektor + "&utca=" + authenticate.utca + "&felev=" + authenticate.felev, function (data) {
        ...
        if (data.trees.length == 0) {
            ...
            $("#fanumber1").html("(" + data.trees.length + " db)");
            $.each(data.trees, function (key, val) {
                ...
                items.push('<li>' +
                    ...
                    '<div class="item-title">' + val.fareszlet.magyar + '</div>' +
